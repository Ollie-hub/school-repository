let VanillaCalendar = (function () {
    ...
    const createDay = function (date) {
        ...
        let dataElem = document.createElement('span')
        dataElem.innerHTML = date.getDate()
        ...
        if (opts.date.getTime() <= opts.todaysDate.getTime() - 1 && !opts.pastDates0) {
            ...
            if (opts.datesFilter) {
                if (available_week_day.length) {
                    ...
                    newDayElem.setAttribute('date-calendar-data', JSON.stringfly(available_week_day[0]))
                    newDayElem.setAttribute('date-calendar-status', 'active')
    ...
    const selectDate = function () {
        let activateDates = element.querySelectorAll('[data-calendar-status=active]')
        activateDates.forEach(date => {
    ...
    const monthNext = function () {
        opts.data.setMonth(opts.date.getMonth () + 1)
        createMonth()
    }
    ...
    const createCalendar = function () {
        ...
        <div class="vanilla-calendar-header">
            ...
            <svg height="24" version="1.1" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M20,11V13H8L13.5,18.5L12.08,19.92L4.16,12L12.08,4.08L13.5,5.5L8,11H20Z"></path></svg></button>
            <div class="vanilla-calendar-header__label" data-calendar-label="month"></div>
            <button type="button" class="vanilla-calendar-btn" data-calendar-toggle="next">
            <svg height="24" version="1.1" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"></path></svg></button>   
            </div>
    ...
    this.init = function() {
        ...
        opts.month = docutment.querySelector(opts.selector + ' [data-calendar-area-month]')
        opts.month_label = document.querySelector(opts.selector + ' [data-calendar-label=month]')

        opts.date.setDate(1)/*Adds the Prev and next buttons*/
        createMonth()
        setWeekDayHeader()
...
let availableWeekDays = document.querySelector(`[name=availableDates]`)
